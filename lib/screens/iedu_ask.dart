...
import 'package:peeroreum_client/screens/iedu_whiteboard.dart';

class AskIedu extends StatefulWidget {
  const AskIedu({super.key});

  @override
  State<AskIedu> createState() => _AskIeduState();
}

class _AskIeduState extends State<AskIedu> {
        ...
        TextButton(
          onPressed: () {
            if (_nextColor == PeeroreumColor.primaryPuple[400]) {
              Fluttertoast.showToast(msg: '완료버튼 ㅎㅎ');
            } else {
              return;
            }
          },
  ...
  Future<void> postImages() async {
    ...
    var response =
        await dio.post('${API.hostConnect}/question/$id', data: formData);
